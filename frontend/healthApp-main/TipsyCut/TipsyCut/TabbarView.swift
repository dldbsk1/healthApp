...
struct MainTabView: View {
    // 💡 1. 값이 변경될 때 뷰를 다시 그리도록 @State로 변경
    @State var userRecommendedCalories: Int = 2000
    @State private var selectedTab: TabMenu = .home
    // 💡 2. 설정 화면 모달(Sheet) 표시 여부를 제어하는 상태 변수 추가
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            Group {
                ...
                case .diet:
                    FoodMainView()
                case .home:
                    // 💡 3. 네비게이션 스택 안에 홈 뷰를 넣고 우측 상단에 톱니바퀴 버튼 추가
                    NavigationStack {
                        HomeView(userRecommendedCalories: userRecommendedCalories)
                            .toolbar {
        ...
        // 💡 4. 설정 화면 모달(Sheet) 연결 로직
        .sheet(isPresented: $showSettings) {
            SettingsView { updatedCalories in
                // 설정 화면에서 저장 성공 시 전달받은 새로운 칼로리로 업데이트
                self.userRecommendedCalories = updatedCalories
            }
        }
    }
