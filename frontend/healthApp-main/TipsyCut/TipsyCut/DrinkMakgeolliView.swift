...
struct DrinkMakgeolliView: View {
    ...
    // 💡 잔 단위 주량 기록 및 칼로리 계산 (막걸리 1잔 = 70kcal)
    @State private var drinkAmount: Double = 1.0
    let makgeolliCupCalories: Double = 70.0
    
    var calculatedDrinkCalories: Double {
        return drinkAmount * makgeolliCupCalories
    }
                ...
                if isLoadingPairing {
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        Button(action: {
                            ...
                            .cornerRadius(12)
                            .shadow(color: Color.brown.opacity(0.3), radius: 6, x: 0, y: 4)
                        }
                    }
                    .padding(20)
                    .background(Color(.systemBackground))
    ...
    private func fetchMakgeolliPairing() {
        Task {
            isLoadingPairing = true
            do {
                let currentUserId = UserDefaults.standard.string(forKey: "userId") ?? ""
                self.recommendedFood = try await NetworkManager.shared.fetchDrinkPairing(drink: "막걸리", userId: currentUserId)
            } catch {
