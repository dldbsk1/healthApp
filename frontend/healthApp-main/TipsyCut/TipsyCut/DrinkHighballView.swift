...
struct DrinkHighballView: View {
    ...
    // 💡 잔 단위 주량 기록 및 칼로리 계산 (하이볼 1잔 = 약 150kcal)
    @State private var drinkAmount: Double = 1.0
    let highballCupCalories: Double = 150.0
    
    var calculatedDrinkCalories: Double {
        return drinkAmount * highballCupCalories
    }
                ...
                if isLoadingPairing {
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        Button(action: {
                            ...
                            .cornerRadius(12)
                            .shadow(color: Color.cyan.opacity(0.3), radius: 6, x: 0, y: 4)
                        }
                    }
                    .padding(20)
                    .background(Color(.systemBackground))
    ...
    private func fetchHighballPairing() {
        Task {
            isLoadingPairing = true
            do {
                let currentUserId = UserDefaults.standard.string(forKey: "userId") ?? ""
                self.recommendedFood = try await NetworkManager.shared.fetchDrinkPairing(drink: "하이볼", userId: currentUserId)
            } catch {
