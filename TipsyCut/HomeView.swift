...
struct HomeView: View {
    ...
    // MARK: - 색상 정의 (Models에 선언된 mintColor 활용)
    let mintColor = Color.mintColor                                  // 메인 민트 (운동 메인)
    let foodColor = Color(red: 0.98, green: 0.92, blue: 0.42)
    //= Color(red: 0.98, green: 0.68, blue: 0.30) //탄단지 섞은 색 (식단 메인)
    //let deepTeal = Color(red: 0.10, green: 0.45, blue: 0.45) // 어두운 청록 (식단 메인)
    
    // MARK: - 탄·단·지 (기존 베이지/코랄핑크/소프트 옐로우 기준 밝고 채도 높인 색상)
    let carbColor = Color(red: 0.98, green: 0.85, blue: 0.60)        // 탄수화물 (더 또렷하고 따뜻한 베이지)
    let proteinColor = Color(red: 0.98, green: 0.50, blue: 0.55)     // 단백질 (밝고 선명해진 코랄 핑크)
    let fatColor = Color(red: 1.00, green: 0.88, blue: 0.35)         // 지방 (밝고 선명한 소프트 옐로우)
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 24) {
                    ...
                    // 이중 도넛 차트 본체
                    ZStack {
                        // 바깥쪽 원: 식단 섭취 칼로리 (foodColor 적용)
                        Circle()
                            .stroke(foodColor.opacity(0.15), lineWidth: 16)
                            .frame(width: 200, height: 200)
                        Circle()
                            .trim(from: 0, to: dietRatio)
                            .stroke(foodColor, style: StrokeStyle(lineWidth: 16, lineCap: .round))
                            .frame(width: 200, height: 200)
                            .rotationEffect(.degrees(-90))
                            
                        // 안쪽 원: 운동 소비 칼로리 (mintColor 적용)
                        Circle()
                ...
                // --- 3. 하단 세부 영양성분 스탯 카드 (탄단지: 선명해진 베이지, 코랄 핑크, 소프트 옐로우) ---
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    VStack(spacing: 18) {
                                        ...
                                        DashboardNutritionBar(label: "탄수화물", value: totalCarb, target: 150.0, color: carbColor)
                                        DashboardNutritionBar(label: "단백질", value: totalProtein, target: 75.0, color: proteinColor)
                                        DashboardNutritionBar(label: "지방", value: totalFat, target: 50.0, color: fatColor)
                                    }
                                }
                                .padding(24)
                                .background(Color(.systemBackground))
