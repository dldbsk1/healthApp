...
from config import YOLO_POSE_ONNX, LSTM_ONNX, YOLO_POSE_PT, LSTM_PTH
from pose_rules import evaluate_pose
from lstm import ExerciseLSTM
...
BASE_DIR = Path(__file__).resolve().parent   # analyzer.py 가 있는 폴더

def load_models():
    pose_model = YOLO(str(BASE_DIR / "weights" / "yolo_ph2_best.pt"))

    device = "cuda" if torch.cuda.is_available() else "cpu"

    lstm_model = ExerciseLSTM()
    lstm_model.load_state_dict(
        torch.load(str(BASE_DIR / "weights" / "exercise_lstm_velo.pth"), map_location=device)
    )
    lstm_model.to(device).eval()

    return pose_model, lstm_model, device, None
# ===================== 메인 분석 =====================
